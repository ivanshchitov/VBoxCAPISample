...
#define USE_ACTIVE_EVENT_LISTENER
        ...
        if (SUCCEEDED(rc) && es)
        {
            ...
            if (SUCCEEDED(rc) && consoleListener)
            {
                ...
                if (SUCCEEDED(rc))
                {
                    ...
                    {
                        IEvent *ev = NULL;
                        rc = IEventSource_GetEvent(es, consoleListener, 500, &ev);
                        if (FAILED(rc))
                        {
...
static void setupFramebuffer(ISession *session)
{
	IConsole *console;
	HRESULT rc = ISession_get_Console(session, &console);
	if (FAILED(rc)) {
		printf("Failed to get session IConsole instance!\n", rc);
		exit(EXIT_FAILURE);
	}
	IMouse *mouse;
	rc = IConsole_get_Mouse(console, &mouse);
	if (FAILED(rc)) {
		printf("Failed to get VBox IMouse instance!\n", rc);
		exit(EXIT_FAILURE);
	}
	IDisplay *display;
	rc = IConsole_get_Display(console, &display);
	if (FAILED(rc)) {
		printf("Failed to get VBox IDisplay instance!\n", rc);
		exit(EXIT_FAILURE);
	}
	printf("Creating FB\n");
	VBoxFramebuffer *framebuffer;

	framebuffer = createFramebuffer_v6_1(display);
	printf("Created FB\n");
	BSTR framebufferUUID;
	rc = IDisplay_AttachFramebuffer(display, 0, (IFramebuffer *) framebuffer, &framebufferUUID);
	printf("Attaching FB, DP: %d, fb: %d, UUID: %d\n", display, framebuffer, framebufferUUID);
	printf("Attached\n");
	if (FAILED(rc)) {
		printf("Failed to attach Framebuffer to VBox IDisplay!\n", rc);
		exit(EXIT_FAILURE);
	}
}
...
static void startVM(const char *argv0, IVirtualBox *virtualBox, ISession *session, BSTR id)
{
    ...
    g_pVBoxFuncs->pfnUtf8ToUtf16("headless", &sessionType);
...
int main(int argc, char **argv)
{
	...
	auto result = AddDllDirectory(L"C:\\Program Files\\Oracle\\VirtualBox\\");
	if (!result) {
		fprintf(stderr, "Unable to add directory\n");
	}
#endif
